...
/**
 * Setup ChatGPT Parts Analysis Workflow
 * 
 * This script helps import the ChatGPT parts analysis workflow into n8n.
...
const WORKFLOW_PATH = path.join(__dirname, '../resources/workflows/chatgpt-parts-analysis.json');
...
async function setupChatGPTWorkflow() {
  try {
    ...
    // Test the webhook (simple test without OpenAI call)
    ...
    try {
      ...
      const testResponse = await axios.post(`${N8N_BASE_URL}/webhook/chatgpt-parts-analysis`, testData);
    ...
    console.log('\n🎉 Setup complete!');
    console.log('📋 Summary:');
    console.log('   - ChatGPT Parts Analysis Workflow created/updated');
    console.log('   - Webhook available at: /webhook/chatgpt-parts-analysis');
    console.log('   - The workflow will analyze parts using AI');
    console.log('\n🔗 Next steps:');
    console.log('   1. Open n8n editor: http://localhost:5678');
    console.log('   2. Add OpenAI credentials to the workflow');
    console.log('   3. Verify the workflow is active (toggle should be ON)');
    console.log('   4. Test the integration in your application');
...
if (require.main === module) {
  setupChatGPTWorkflow();
}

module.exports = setupChatGPTWorkflow; 